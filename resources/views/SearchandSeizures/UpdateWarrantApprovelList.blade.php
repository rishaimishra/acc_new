      ...
      <div class = "col-12">
        ...
        <div class = "card">
          <div class = "card-header">
            <h2 class = "card-title text-info"><b>Search Details-Update Warrant Approvel</b></h2>
          </div>
            ...
            <table id = "example3" class="table table-bordered table-hover ">
              <thead class="bg-info text-center">
                <tr>
                  ...
                  <th scope="col">Warrant Approvel</th> 
              ...
              <tbody>
                <tr>
                  ...
                  <th>{{ ++$i }}</th>
                    ...
                    <td>
                      @if(!empty($data->warrantNo))
                      <label class="text-success">Approved</label>
                      @else
                      <label class="text-warning">Rejected/Pending</label>
                      @endif
                    </td>
                    {{-- <td>
                      @if($data->directorStatus=='Recommended')
                        <a class="btn btn-info btn-sm text-white">Recommended</a></td>
                      @elseif($data->directorStatus=='Reject')
                        <a class="btn btn-danger btn-sm text-white">Rejected</a></td>
                      @elseif($data->directorStatus==0)
                        <a class="btn btn-warning btn-sm text-white">Pending</a></td>
                      @endif --}}
                      <td>
                        <a href="{{route('UpdateWarrantApprovelView',$data->seizure_id) }}" class="btn btn-info btn-sm text-white">Update</a>      
                      </td>
                </tr>
                @endforeach
              </tbody>
